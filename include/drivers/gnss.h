...
struct gnss_position {
	...
	struct {
		uint8_t gnssFixOK : 1; // valid fix (ie: within DOP nad accuracy masks)
		uint8_t diffSoln : 1; // differential corrections were applied
		uint8_t psmState : 4;
		uint8_t headVehValid : 1; // heading of vehicle is valid (only set if the
			// receivers is in sensor fusion mode)
		uint8_t carrSoln : 1; // carrier phase range solution status
...
struct gnss_velocity {
	...
	int32_t heading_of_motion; // degrees * 10^-5
...
struct gnss_status {
	uint32_t tow; // GPS time of week of the navigation epoch
	uint32_t ttff; // (ms) time to first fix
	uint32_t uptime; // (ms) device uptime
...
// different arrays
enum gnss_sentence_type { NONE = 0, NMEA, UBX, RTCM };
...
enum gnss_model // Possible values for the dynamic platform model, which provide
// more accuract position output for the situation. Description
// extracted from ZED-F9P Integration Manual
{
	GNSS_MODEL_PORTABLE = 0, // Applications with low acceleration, e.g. portable
	// devices. Suitable for most situations.
	// 1 is not defined
	GNSS_MODEL_STATIONARY =
	...
	// assumed.
	GNSS_MODEL_PEDESTRIAN, // Applications with low acceleration and speed, e.g.
	// how a pedestrian would move. Low acceleration
	// assumed.
	GNSS_MODEL_AUTOMOTIVE, // Used for applications with equivalent dynamics to
	// those of a passenger car. Low vertical acceleration
	// assumed
	GNSS_MODEL_SEA, // Recommended for applications at sea, with zero vertical
	// velocity. Zero vertical velocity assumed. Sea level
	// assumed.
	GNSS_MODEL_AIRBORNE1g, // Airborne <1g acceleration. Used for applications
	// with a higher dynamic range and greater vertical
	// acceleration than a passenger car. No 2D position
	// fixes supported.
	GNSS_MODEL_AIRBORNE2g, // Airborne <2g acceleration. Recommended for typical
	// airborne environments. No 2D position fixes
	// supported.
	GNSS_MODEL_AIRBORNE4g, // Airborne <4g acceleration. Only recommended for
	// extremely dynamic environments. No 2D position fixes
	// supported.
	GNSS_MODEL_WRIST, // Not supported in protocol versions less than 18. Only
 ...
 */
typedef void (*gnss_trigger_handler_t)(struct device *dev, struct gnss_trigger *trigger);
 ...
 */
typedef int (*gnss_trigger_set_t)(struct device *dev, const struct gnss_trigger *trig,
 ...
 */
typedef int (*gnss_attr_get_t)(struct device *dev, enum gnss_channel chan, enum gnss_attribute attr,
			       void *val);
 ...
 */
typedef int (*gnss_attr_set_t)(struct device *dev, enum gnss_channel chan, enum gnss_attribute attr,
			       void *val);
/**
 ...
 */
typedef int (*gnss_sample_fetch_t)(struct device *dev, enum gnss_channel chan);
/**
 ...
 */
typedef int (*gnss_channel_get_t)(struct device *dev, enum gnss_channel chan, void *val);
 ...
 * See gnss_attr_get() for argument description
 */
typedef int (*gnss_enable_t)(struct device *dev, gnss_status_callback_t handler);
 ...
 */
__syscall int gnss_attr_get(struct device *dev, enum gnss_channel chan, enum gnss_attribute attr,
			    void *val);

static inline int z_impl_gnss_attr_get(struct device *dev, enum gnss_channel chan,
				       enum gnss_attribute attr, void *val)
{
 ...
 * @return 0 if successful, negative errno code if failure.
 */
__syscall int gnss_attr_set(struct device *dev, enum gnss_channel chan, enum gnss_attribute attr,
			    void *val);

static inline int z_impl_gnss_attr_set(struct device *dev, enum gnss_channel chan,
				       enum gnss_attribute attr, void *val)
{
	const struct gnss_driver_api *api = (const struct gnss_driver_api *)dev->api;
 ...
 * @return 0 if successful, negative errno code if failure.
 */
static inline int gnss_trigger_set(struct device *dev, struct gnss_trigger *trig,
				   gnss_trigger_handler_t handler)
{
	const struct gnss_driver_api *api = (const struct gnss_driver_api *)dev->api;
 ...
 * @return 0 if successful, negative errno code if failure.
 */
__syscall int gnss_sample_fetch_chan(struct device *dev, enum gnss_channel type);

static inline int z_impl_gnss_sample_fetch_chan(struct device *dev, enum gnss_channel type)
{
	const struct gnss_driver_api *api = (const struct gnss_driver_api *)dev->api;
 ...
 * @return 0 if successful, negative errno code if failure.
 */
__syscall int gnss_channel_get(struct device *dev, enum gnss_channel chan, void *val);

static inline int z_impl_gnss_channel_get(struct device *dev, enum gnss_channel chan, void *val)
{
	const struct gnss_driver_api *api = (const struct gnss_driver_api *)dev->api;
 ...
 * @return 0 if successful, negative errno code if failure.
 */
static inline int gnss_enable(struct device *dev, gnss_status_callback_t status_cb)
{
	const struct gnss_driver_api *api = (const struct gnss_driver_api *)dev->api;
