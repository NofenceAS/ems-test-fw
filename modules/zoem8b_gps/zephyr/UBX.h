...
#define BYTESWAP32(num)                                                                            \
	(((num >> 24) & 0xff) | ((num << 8) & 0xff0000) | ((num >> 8) & 0xff00) |                  \
	 ((num << 24) & 0xff000000))
...
The default is the WGS84 Ellipsoid, but  can be changed with the message <r
href=\"CFG-DAT'>CFG-DAT</r>.
...
#define GPS_UBX_NAV_STATUS_FLAGS_GPSFIXOK_GET(val)                                                 \
	(U)(((val)&GPS_UBX_NAV_STATUS_FLAGS_GPSFIXOK_MASK) >>                                      \
	    0) //!< Get gpsFixOk from bitmask flags
...
#define GPS_UBX_NAV_STATUS_FLAGS_DIFFSOLN_GET(val)                                                 \
	(U)(((val)&GPS_UBX_NAV_STATUS_FLAGS_DIFFSOLN_MASK) >>                                      \
	    1) //!< Get diffSoln from bitmask flags
...
* All DOP values are scaled by a factor of 100. that is, if the unit transmits a
value of e.g. 156, it means that the DOP value is 1.56.
...
This message is combiningPosition, velocity and time solution in ECEF,including
accuracy figures
...
#define GPS_UBX_NAV_TIMEUTC_VALID_VALIDTOW_GET(val)                                                \
	(U)(((val)&GPS_UBX_NAV_TIMEUTC_VALID_VALIDTOW_MASK) >>                                     \
	    0) //!< Get validTOW from bitmask valid
...
#define GPS_UBX_NAV_TIMEUTC_VALID_VALIDWKN_GET(val)                                                \
	(U)(((val)&GPS_UBX_NAV_TIMEUTC_VALID_VALIDWKN_MASK) >>                                     \
	    1) //!< Get validWKN from bitmask valid
...
#define GPS_UBX_NAV_TIMEUTC_VALID_VALIDUTC_GET(val)                                                \
	(U)(((val)&GPS_UBX_NAV_TIMEUTC_VALID_VALIDUTC_MASK) >>                                     \
	    2) //!< Get validUTC from bitmask valid
...
#define GPS_UBX_NAV_SVINFO_GLOBALFLAGS_ISU5_GET(val)                                               \
	(U)(((val)&GPS_UBX_NAV_SVINFO_GLOBALFLAGS_ISU5_MASK) >>                                    \
	    0) //!< Get isU5 from bitmask globalFlags
...
#define GPS_UBX_NAV_SVINFO_CHN_FLAGS_SVUSED_GET(val)                                               \
	(U)(((val)&GPS_UBX_NAV_SVINFO_CHN_FLAGS_SVUSED_MASK) >>                                    \
	    0) //!< Get svUsed from bitmask flags
...
#define GPS_UBX_NAV_SVINFO_CHN_FLAGS_DIFFCORR_GET(val)                                             \
	(U)(((val)&GPS_UBX_NAV_SVINFO_CHN_FLAGS_DIFFCORR_MASK) >>                                  \
	    1) //!< Get diffCorr from bitmask flags
...
#define GPS_UBX_NAV_SVINFO_CHN_FLAGS_ORBITEPH_GET(val)                                             \
	(U)(((val)&GPS_UBX_NAV_SVINFO_CHN_FLAGS_ORBITEPH_MASK) >>                                  \
	    3) //!< Get orbitEph from bitmask flags
...
#define GPS_UBX_NAV_SVINFO_CHN_FLAGS_ORBITALM_GET(val)                                             \
	(U)(((val)&GPS_UBX_NAV_SVINFO_CHN_FLAGS_ORBITALM_MASK) >>                                  \
	    5) //!< Get orbitAlm from bitmask flags
...
#define GPS_UBX_NAV_SBAS_SERVICE_RANGING_GET(val)                                                  \
	(U)(((val)&GPS_UBX_NAV_SBAS_SERVICE_RANGING_MASK) >>                                       \
	    0) //!< Get Ranging from bitmask service
...
#define GPS_UBX_NAV_SBAS_SERVICE_INTEGRITY_GET(val)                                                \
	(U)(((val)&GPS_UBX_NAV_SBAS_SERVICE_INTEGRITY_MASK) >>                                     \
	    2) //!< Get Integrity from bitmask service
...
#define GPS_UBX_NAV_SBAS_SERVICE_TESTMODE_GET(val)                                                 \
	(U)(((val)&GPS_UBX_NAV_SBAS_SERVICE_TESTMODE_MASK) >>                                      \
	    3) //!< Get Testmode from bitmask service
...
+For GPS satellites, the 10 dwrd values contain the parity checked subframe data
for 10 Words. Each dwrd has 24 Bits with valid data (Bits 23 to 0). The
remaining 8 bits (31 to 24) have an undefined value. The direction within the
Word is that the higher order bits are received from the SV first. Example: The
Preamble can be found in dwrd[0], at bit position 23 down to 16. For more
details on the data format please refer to the ICD-GPS-200C Interface document.
...
Bits 25 and 24 are the last two data bits, and Bits 23 down to 0 are the parity
bits. For more information on SBAS data format, please refer to RTCA/DO-229C
(MOPS), Appendix A.
...
#define GPS_UBX_RXM_SVSI_SVID_SVFLAG_HEALTHY_GET(val)                                              \
	(U)(((val)&GPS_UBX_RXM_SVSI_SVID_SVFLAG_HEALTHY_MASK) >>                                   \
	    4) //!< Get healthy from bitmask svFlag
...
#define GPS_UBX_RXM_SVSI_SVID_SVFLAG_EPHVAL_GET(val)                                               \
	(U)(((val)&GPS_UBX_RXM_SVSI_SVID_SVFLAG_EPHVAL_MASK) >>                                    \
	    5) //!< Get ephVal from bitmask svFlag
...
#define GPS_UBX_RXM_SVSI_SVID_SVFLAG_ALMVAL_GET(val)                                               \
	(U)(((val)&GPS_UBX_RXM_SVSI_SVID_SVFLAG_ALMVAL_MASK) >>                                    \
	    6) //!< Get almVal from bitmask svFlag
...
#define GPS_UBX_RXM_SVSI_SVID_SVFLAG_NOTAVAIL_GET(val)                                             \
	(U)(((val)&GPS_UBX_RXM_SVSI_SVID_SVFLAG_NOTAVAIL_MASK) >>                                  \
	    7) //!< Get notAvail from bitmask svFlag
...
typedef struct __packed {
	U1 version;
	U1 reserved1[3];
	U4 duration; // Duration of the requested task, set to zero for infinite
		// duration
	X4 flags; // Flags
	X4 wakeupsource;
...
#define GPS_UBX_RXM_PMREQ_FLAGS_BACKUP_MASK                                                        \
	0x2 //!< Mask for field awake in bitmask flags. means not in backup mode                   \
		//!< (incative state)
//#define GPS_UBX_RXM_PMREQ_FLAGS_BACKUP_GET(val)
//(U)(((val)&GPS_UBX_RXM_PMREQ_FLAGS_BACKUP_MASK)>>0)  //!< Get awake bit from
//bitmask flags
...
typedef struct __packed // Polls the configuration of the used I/O Port
{
...
the payload length can be a multiple of the normal length. Output messages from
the module contain only one configuration unit. Note that some fields are
...
#define GPS_UBX_CFG_PRT_UART_U5_MODE_CHARLEN_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_PRT_UART_U5_MODE_CHARLEN_MASK) >>                                   \
	    6) //!< Get charLen from bitmask mode
...
typedef struct __packed // Get/Set Port Configuration for USB Port
{
...
#define GPS_UBX_CFG_PRT_SPI_U5_MODE_SPIMODE_GET(val)                                               \
	(U)(((val)&GPS_UBX_CFG_PRT_SPI_U5_MODE_SPIMODE_MASK) >>                                    \
	    1) //!< Get spiMode from bitmask mode
...
#define GPS_UBX_CFG_PRT_I2C_U5_MODE_SLAVEADDR_GET(val)                                             \
	(U)(((val)&GPS_UBX_CFG_PRT_I2C_U5_MODE_SLAVEADDR_MASK) >>                                  \
	    1) //!< Get slaveAddr from bitmask mode
...
*  Several configurations can be concatenated to one input message. In this case
the payload length can be a multiple of the normal length. Output messages from
the module contain only one configuration unit.
...
configurations can be concatenated to one input message. In this case the
payload length can be a multiple of the normal length. Output messages from the
module contain only one configuration unit. Please note that I/O Targets 0, 1
and 2 correspond to serial ports 0, 1 and 2. I/O target 3 is reserved for future
use.
...
typedef struct __packed {
	...
	U1 res0; //!< Reserved
	U2 res1; //!< Reserved
	X1 infMsgMask[6]; //!< A bit mask, saying which information messages are
...
#define GPS_UBX_CFG_INF_INFMSGMASK_ERROR_GET(val)                                                  \
	(U)(((val)&GPS_UBX_CFG_INF_INFMSGMASK_ERROR_MASK) >>                                       \
	    0) //!< Get ERROR from bitmask infMsgMask
...
#define GPS_UBX_CFG_INF_INFMSGMASK_WARNING_GET(val)                                                \
	(U)(((val)&GPS_UBX_CFG_INF_INFMSGMASK_WARNING_MASK) >>                                     \
	    1) //!< Get WARNING from bitmask infMsgMask
...
#define GPS_UBX_CFG_INF_INFMSGMASK_NOTICE_GET(val)                                                 \
	(U)(((val)&GPS_UBX_CFG_INF_INFMSGMASK_NOTICE_MASK) >>                                      \
	    2) //!< Get NOTICE from bitmask infMsgMask
...
#define GPS_UBX_CFG_INF_INFMSGMASK_DEBUG_GET(val)                                                  \
	(U)(((val)&GPS_UBX_CFG_INF_INFMSGMASK_DEBUG_MASK) >>                                       \
	    3) //!< Get DEBUG from bitmask infMsgMask
...
#define GPS_UBX_CFG_RST_NAVBBRMASK_HEALTH_GET(val)                                                 \
	(U)(((val)&GPS_UBX_CFG_RST_NAVBBRMASK_HEALTH_MASK) >>                                      \
	    2) //!< Get health from bitmask navBbrMask
...
The three masks are made up of individual bits, each bit indicating the section
ID on which the corresponding action shall be carried out.Please note that
commands can be combined. The sequence of execution is Clear, Save, Load
...
typedef struct __packed {
	...
	X4 loadMask; //!< Mask with configuration ids to Load (=Load Settings from
		//!< Non-volatile Memory to Active Settings), see ID description
		//!< of clearMask
} GPS_UBX_CFG_CFG_t;
...
#define GPS_UBX_CFG_CFG_CLEARMASK_IOPORT_GET(val)                                                  \
	(U)(((val)&GPS_UBX_CFG_CFG_CLEARMASK_IOPORT_MASK) >>                                       \
	    0) //!< Get ioPort from bitmask clearMask
...
#define GPS_UBX_CFG_CFG_CLEARMASK_MSGCONF_GET(val)                                                 \
	(U)(((val)&GPS_UBX_CFG_CFG_CLEARMASK_MSGCONF_MASK) >>                                      \
	    1) //!< Get msgConf from bitmask clearMask
...
#define GPS_UBX_CFG_CFG_CLEARMASK_INFMSG_GET(val)                                                  \
	(U)(((val)&GPS_UBX_CFG_CFG_CLEARMASK_INFMSG_MASK) >>                                       \
	    2) //!< Get infMsg from bitmask clearMask
...
#define GPS_UBX_CFG_CFG_CLEARMASK_NAVCONF_GET(val)                                                 \
	(U)(((val)&GPS_UBX_CFG_CFG_CLEARMASK_NAVCONF_MASK) >>                                      \
	    3) //!< Get navConf from bitmask clearMask
...
#define GPS_UBX_CFG_CFG_CLEARMASK_RXMCONF_GET(val)                                                 \
	(U)(((val)&GPS_UBX_CFG_CFG_CLEARMASK_RXMCONF_MASK) >>                                      \
	    4) //!< Get rxmConf from bitmask clearMask
...
#define GPS_UBX_CFG_CFG_CLEARMASK_LPMCONF_GET(val)                                                 \
	(U)(((val)&GPS_UBX_CFG_CFG_CLEARMASK_LPMCONF_MASK) >>                                      \
	    5) //!< Get lpmConf from bitmask clearMask
...
#define GPS_UBX_CFG_CFG_CLEARMASK_ANTCONF_GET(val)                                                 \
	(U)(((val)&GPS_UBX_CFG_CFG_CLEARMASK_ANTCONF_MASK) >>                                      \
	    10) //!< Get antConf from bitmask clearMask

#define UBXID_CFG_CFG 0x0609 //!< message id for CFG-CFG

#define UBXID_CFG_FXN 0x060E
typedef struct __packed // Poll FXN configuration
{
} GPS_UBX_CFG_FXN_POLL_t;
typedef struct __packed // RXM FixNOW configuration.
{
...
typedef struct __packed // RXM configuration
{
	U1 reserved; // reserved
	U1 lpMode; // Low Power Mode
} GPS_UBX_CFG_RXM_t;

typedef enum { UBX_CFG_RXM_MODE_CONTINOUS = 0, UBX_CFG_RXM_MODE_POWER_SAVE = 1 } UBX_CFG_RXM_MODE_t;
...
/*!
SBAS Configuration
This message configures the SBAS receiver subsystem. (i.e. WAAS, EGNOS, MSAS)For
detailed description of the SBAS services, please refer to Document RTCA/DO-229C
(MOPS), available from http://www.rtca.org/
...
#define GPS_UBX_CFG_SBAS_U5_USAGE_DIFFCORR_GET(val)                                                \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_USAGE_DIFFCORR_MASK) >>                                     \
	    1) //!< Get diffCorr from bitmask usage
...
#define GPS_UBX_CFG_SBAS_U5_USAGE_INTEGRITY_GET(val)                                               \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_USAGE_INTEGRITY_MASK) >>                                    \
	    2) //!< Get integrity from bitmask usage
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN152_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN152_MASK) >>                                   \
	    0) //!< Get PRN152 from bitmask scanmode2
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN153_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN153_MASK) >>                                   \
	    1) //!< Get PRN153 from bitmask scanmode2
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN154_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN154_MASK) >>                                   \
	    2) //!< Get PRN154 from bitmask scanmode2
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN155_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN155_MASK) >>                                   \
	    3) //!< Get PRN155 from bitmask scanmode2
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN156_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN156_MASK) >>                                   \
	    4) //!< Get PRN156 from bitmask scanmode2
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN157_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN157_MASK) >>                                   \
	    5) //!< Get PRN157 from bitmask scanmode2
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN158_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE2_PRN158_MASK) >>                                   \
	    6) //!< Get PRN158 from bitmask scanmode2
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN120_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN120_MASK) >>                                   \
	    0) //!< Get PRN120 from bitmask scanmode1
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN121_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN121_MASK) >>                                   \
	    1) //!< Get PRN121 from bitmask scanmode1
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN122_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN122_MASK) >>                                   \
	    2) //!< Get PRN122 from bitmask scanmode1
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN123_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN123_MASK) >>                                   \
	    3) //!< Get PRN123 from bitmask scanmode1
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN124_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN124_MASK) >>                                   \
	    4) //!< Get PRN124 from bitmask scanmode1
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN125_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN125_MASK) >>                                   \
	    5) //!< Get PRN125 from bitmask scanmode1
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN126_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN126_MASK) >>                                   \
	    6) //!< Get PRN126 from bitmask scanmode1
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN127_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN127_MASK) >>                                   \
	    7) //!< Get PRN127 from bitmask scanmode1
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN128_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN128_MASK) >>                                   \
	    8) //!< Get PRN128 from bitmask scanmode1
...
#define GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN129_GET(val)                                              \
	(U)(((val)&GPS_UBX_CFG_SBAS_U5_SCANMODE1_PRN129_MASK) >>                                   \
	    9) //!< Get PRN129 from bitmask scanmode1
...
#define GPS_UBX_CFG_NMEA_FILTER_MSKPOSFILT_GET(val)                                                \
	(U)(((val)&GPS_UBX_CFG_NMEA_FILTER_MSKPOSFILT_MASK) >>                                     \
	    1) //!< Get mskPosFilt from bitmask filter
...
#define GPS_UBX_CFG_NMEA_FILTER_TIMEFILT_GET(val)                                                  \
	(U)(((val)&GPS_UBX_CFG_NMEA_FILTER_TIMEFILT_MASK) >>                                       \
	    2) //!< Get timeFilt from bitmask filter
...
#define GPS_UBX_CFG_NMEA_FILTER_DATEFILT_GET(val)                                                  \
	(U)(((val)&GPS_UBX_CFG_NMEA_FILTER_DATEFILT_MASK) >>                                       \
	    3) //!< Get dateFilt from bitmask filter
...
#define GPS_UBX_CFG_NMEA_FILTER_SBASFILT_GET(val)                                                  \
	(U)(((val)&GPS_UBX_CFG_NMEA_FILTER_SBASFILT_MASK) >>                                       \
	    4) //!< Get sbasFilt from bitmask filter
...
#define GPS_UBX_CFG_NMEA_FILTER_TRACKFILT_GET(val)                                                 \
	(U)(((val)&GPS_UBX_CFG_NMEA_FILTER_TRACKFILT_MASK) >>                                      \
	    5) //!< Get trackFilt from bitmask filter
...
typedef struct __packed // Poll Navigation Engine Expert Settings
{
} GPS_UBX_CFG_NAVX5_POLL_t;

typedef struct __packed // Get/Set Navigation Engine Expert Settings
{
...
Description</r> for adetailed description of how these settings affect receiver
operation.
...
#define GPS_UBX_CFG_NAV5_MASK_DGPSMASK_GET(val)                                                    \
	(U)(((val)&GPS_UBX_CFG_NAV5_MASK_DGPSMASK_MASK) >>                                         \
	    7) //!< Get staticHoldMask from bitmask mask
//#define UBXID_CFG_NAV5 0x0624  // already defined, see above

#define UBXID_CFG_TP5 0x0631
typedef struct __packed // Poll Timepulse Parameters
{
...
typedef struct __packed // Get/Set TimePulse Parameters
{
	U1 tpIdx; // Timepulse selection (0 = TIMEPULSE, 1 =
	U1 res0; // Reserved
	U2 res1; // Reserved
...
typedef struct __packed // Power Management configuration
{
...
typedef struct __packed // Power Mode Setup
{
...
typedef struct __packed // Set/Get contents of Remote Inventory
{
...
#define GPS_UBX_CFG_ITFM_CONFIG_RESERVED1_SET                                                      \
	((X4)0x16B156 << 9) //!< Set reserved1 (constant) into config
#define GPS_UBX_CFG_ITFM_CONFIG_ENABLE_SET ((X4)1 << 31) //!< Set enable bit into config

#define GPS_UBX_CFG_ITFM_CONFIG2_RESERVED2_SET                                                     \
	((X4)0x31E) //!< Set reserved2 (constant) into config2
#define GPS_UBX_CFG_ITFM_CONFIG2_ANTSETTING_SET(val)                                               \
	((X4)val << 12) //!< Set antSetting into config2
#define GPS_UBX_CFG_ITFM_CONFIG2_RESERVED3_SET                                                     \
	((U4)0x00 << 14) //!< Set reserved3 (constant) into config2
...
supports.i.e. on Antaris this is always 4, on u-blox 5 the number of ports is 6.
...
typedef struct __packed {
	...
	U1 jamInd; //!< CW Jamming indicator, scaled (0 = no CW jamming, 255 = strong
		//!< CW jamming)
	U2 res3; //!< Reserved
...
#define GPS_UBX_MON_HW_FLAGS_RTCCALIB_GET(val)                                                     \
	(U)(((val)&GPS_UBX_MON_HW_FLAGS_RTCCALIB_MASK) >> 0) //!< Get rtcCalib from bitmask flags
...
The receiver ready message is sent when the receiver changes from or to backup
mode.
...
#define GPS_UBX_MON_RXR_FLAGS_AWAKE_MASK                                                           \
	0x1 //!< Mask for field awake in bitmask flags. means not in backup mode                   \
		//!< (incative state)
#define GPS_UBX_MON_RXR_FLAGS_AWAKE_GET(val)                                                       \
	(U1)(((val)&GPS_UBX_MON_RXR_FLAGS_AWAKE_MASK) >> 0) //!< Get awake bit from bitmask flags
...
more information on these parameters, please see the ICD-GPS-200 documentation.
...
If this poll is received, the messages AID-INI, AID-HUI, AID-EPH and AID-ALM are
sent (Antaris only). u-blox 5 uses this message just to poll.
...
* 	If the WEEK Value is 0, DWRD0 to DWRD7 are not sent as the almanach is
not available for the given SV.
* 	DWORD0 to DWORD7 contain the 8 words following the Hand-Over Word ( HOW
) from the GPS navigation message, either pages 1 to 24 of sub-frame 5 or pages
2 to 10 of subframe 4. See ICD-GPS-200 for a full description of the contents of
the Almanac pages.
* 	In DWORD0 to DWORD7, the parity bits have been removed, and the 24 bits
of data are located in Bits 0 to 23. Bits 24 to 31 are the sign-extension of the
data.
* 	Example: Parameter e (Eccentricity) from Almanach Subframe 4/5, Word 3,
Bits 69-84 within the subframe can be found in DWRD0, Bits 15-0 whereas Bit 0 is
the LSB.
...
Poll GPS Constellation Data (Ephemeris) for an SV by sending this message to the
receiver. The receiver will return one message of type AID-EPH as defined below.
...
payload is reduced to 4 Bytes, indicating that this SV Number does not have
valid ephemeris for the moment.
* SF1D0 to SF3D7  contain the 24 words following the Hand-Over Word ( HOW ) from
the GPS navigation message, subframes 1 to 3. See ICD-GPS-200 for a full
...
This message is sent by the ALP server to the ALP client and is usually sent in
response to a data request. The server copies the identifier from the request
and fills in the dataSize and fileId fields.
...
updated data. The server can either replace the current data at this position or
choose to ignore this new data (which will result in degraded performance).
...
the receiver. Upon reception of this message, the receiver will write the
payload data to its internal non-volatile memory, eventually also erasing that
part of the memory first. Make sure that the payload size is even sized (i.e.
always a multiple of 2). Do not use payloads larger than ~ 700 bytes, as this
would exceed the receiver�s internal buffering capabilities. The receiver will
(not-) acknowledge this message using the message alternatives given below. The
host shall wait for an acknowledge message before sending the next chunk.
...
/*!
Timepulse Timedata
This message contains information for high precision timing. Note that contents
are correct only if the timepulse is set to one pulse per second.
...
counting. +The delay figures given in <r href=\"CFG-TP'>CFG-TP</r> are also
applied to the time results output in this message.
