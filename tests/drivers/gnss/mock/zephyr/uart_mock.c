...
static int mock_uart_configure(const struct device *dev, const struct uart_config *cfg)
{
...
/* See UART driver API for details */
static int mock_uart_config_get(const struct device *dev, struct uart_config *cfg)
{
#ifdef MOCK_UART_VERBOSE
...
/* See UART driver API for details */
static int mock_uart_fifo_fill(const struct device *dev, const uint8_t *tx_data, int len)
{
	(void)dev;
...
/* See UART driver API for details */
static int mock_uart_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	...
	uint8_t *data;
	uint32_t cnt = ring_buf_get_claim(&uart_mock_rx_ring_buf, &data, UART_MOCK_RX_SIZE);
...
{
	return mock_uart_irq_rx_ready(dev) || mock_uart_irq_tx_ready(dev) ||
...
/* See UART driver API for details */
static void mock_uart_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
				       void *cb_data)
{
	(void)dev;
	irq_callback = cb;
...
{
	ring_buf_init(&uart_mock_tx_ring_buf, UART_MOCK_TX_SIZE, uart_mock_tx_buffer);

	ring_buf_init(&uart_mock_rx_ring_buf, UART_MOCK_RX_SIZE, uart_mock_rx_buffer);
...
static struct mock_uart_data data_mock = { .uart_config = {
...
DEVICE_DT_INST_DEFINE(0, mock_uart_init, NULL, &data_mock, NULL,
		      /* Initialize UART device before UART console. */
		      PRE_KERNEL_1, 10, &mock_uart_driver_api);
