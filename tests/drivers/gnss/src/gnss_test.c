...
/* GNSS lastfix data callback, copies data and signals via sem */
...
static void test_receive_gnss_solution(void)
{
	...
	zassert_equal(gnss_set_lastfix_cb(gnss_dev, gnss_lastfix_cb), 0, 
		      "Tried registering data callback, got error");
	...
	zassert_equal(gnss_lastfix_fetch(gnss_dev, &gnss_fetch_lastfix), 
		      -ENODATA, "Wrong error code for no GNSS lastfix");
	...
	zassert_equal(k_sem_take(&gnss_lastfix_sem, K_MSEC(1000)), 0, 
		      "Did not get GNSS lastfix callback");
	...
	zassert_equal(gnss_lastfix_fetch(gnss_dev, &gnss_fetch_lastfix), 0, 
		      "Fetching GNSS lastfix failed");

	zassert_mem_equal(&gnss_fetch_data, &gnss_cb_data, 
			  sizeof(gnss_struct_t), 
			  "Mismatch between GNSS data in callback and fetch");
	zassert_mem_equal(&gnss_fetch_lastfix, &gnss_cb_lastfix, 
			  sizeof(gnss_last_fix_struct_t), 
			  "Mismatch between GNSS lastfix in callback and fetch");
}
...
static void test_ignore_nmea(void)
{
	...
	zassert_equal(k_sem_take(&gnss_data_sem, K_NO_WAIT), -EBUSY , 
		      "Unexpected data");
	zassert_equal(k_sem_take(&gnss_lastfix_sem, K_NO_WAIT), -EBUSY , 
		      "Unexpected lastfix");
	...
	zassert_equal(k_sem_take(&gnss_data_sem, K_MSEC(100)), 0 , 
		      "No data received");
	zassert_equal(k_sem_take(&gnss_lastfix_sem, K_MSEC(100)), 0 , 
		      "No lastfix received");
