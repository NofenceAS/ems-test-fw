...
static void test_encode_valid_data(void) 
{
	...
	result = collar_protocol_encode(&reqMsg, raw_buf, sizeof(raw_buf), &raw_size);
	zassert_equal(result, 0, "collar_protocol_encode failed for valid data");

	result = collar_protocol_decode(raw_buf, raw_size, &message);
	zassert_equal(result, 0 ,"collar_protocol_decode failed for valid data");

	result = memcmp(&reqMsg, &message, sizeof(NofenceMessage));
	zassert_equal(result, 0, "collar_protocol_decode resulted in unexpected data");
}
...
static void test_decode_valid_data(void)
{
	...
	zassert_true(message.header.has_ulVersion, "collar_protocol_decode did not decode ulVersion");
	zassert_equal(message.header.ulVersion, 327, "collar_protocol_decode return wrong ulVersion");
	zassert_equal(message.header.ulId, 1337, "collar_protocol_decode return wrong ulId");
	zassert_equal(message.header.ulUnixTimestamp, 1638999508, "collar_protocol_decode return wrong ulUnixTimestamp");
	zassert_equal(message.which_m, NofenceMessage_firmware_upgrade_req_tag, "collar_protocol_decode return wrong which_m");
	zassert_equal(message.m.firmware_upgrade_req.ulVersion, 929598, "collar_protocol_decode return wrong ulVersion");
	zassert_true(message.m.firmware_upgrade_req.has_ucHardwareVersion, "collar_protocol_decode did not decode ucHardwareVersion");
	zassert_equal(message.m.firmware_upgrade_req.ucHardwareVersion, 14, "collar_protocol_decode return wrong ucHardwareVersion");
	zassert_true(message.m.firmware_upgrade_req.has_versionInfoHW, "collar_protocol_decode did not decode versionInfoHW");
	zassert_equal(message.m.firmware_upgrade_req.versionInfoHW.ucPCB_RF_Version, 12, "collar_protocol_decode return wrong ucPCB_RF_Version");
	zassert_equal(message.m.firmware_upgrade_req.versionInfoHW.usPCB_Product_Type, 1, "collar_protocol_decode return wrong usPCB_Product_Type");
	zassert_equal(message.m.firmware_upgrade_req.versionInfoHW.ucPCB_HV_Version, 0, "collar_protocol_decode return wrong ucPCB_HV_Version");
	zassert_equal(message.m.firmware_upgrade_req.usFrameNumber, 16, "collar_protocol_decode return wrong usFrameNumber");
}
...
static void test_invalid_input(void) {
	...
	zassert_equal(result, -EINVAL, "collar_protocol_decode returned wrong error for invalid arguments");
	...
	result = collar_protocol_decode(raw_buf, sizeof(raw_buf), &message);
	zassert_equal(result, 0, "collar_protocol_decode returned unexpected error");

	// Test encode with invalid input arguments
	result = collar_protocol_encode(&message, raw_buf, sizeof(raw_buf), NULL);
	zassert_equal(result, -EINVAL, "collar_protocol_encode returned wrong error for invalid arguments");
...
static void test_buffer_overflow(void) {
	...
	result = collar_protocol_encode(&message, raw_buf, sizeof(raw_buf)-1, &raw_size);
	zassert_equal(result, -EMSGSIZE, "collar_protocol_encode returned wrong result for insufficient buffer size (-1)");

	// Attempt encode with buffer size half of required
	result = collar_protocol_encode(&message, raw_buf, sizeof(raw_buf)/2, &raw_size);
	zassert_equal(result, -EMSGSIZE, "collar_protocol_encode returned wrong result for insufficient buffer size (/2)");
...
static void test_corrupted_decode(void) {
	...
	result = collar_protocol_decode(corrupted_raw_buf, sizeof(corrupted_raw_buf), &message);
	
	zassert_equal(result, -EILSEQ, "collar_protocol_decode returned wrong result code for corrupted data input");
}
