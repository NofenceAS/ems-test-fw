...
void test_correction_mode(void)
{
	...
	*/
	...
	/*
	* Verify that all collar modes that should not start AMC correction 
	* don't start AMC correction.
	*/
...
void test_correction_fence_status(void)
{
	...
	*/
	...
	/*
	* Verify that all fence statuses that should not start AMC correction 
	* does not start AMC correction.
	*/
...
void test_correction_zone(void)
{
	/* 
	* Test that AMC correction is started/not started based on the correct 
	* collar modes accordingly. Correction shall only started when Collar 
	* Mode is Mode_Fence and Mode_Teach.
	*/
...
{
	/*
	* Test that AMC correction is started, not started or paused based on 
	* the state of GNSS data input.
	*/
	...
	/*
	* Test 1
	* Test that an accepted GNSS fix does not start correction, only a 
	* warning GNSS fix is accepted for correction to start.
	*/
	...
	*/
	...
	/* GNSS modes other than GNSSMODE_MAX cannot start correction */
	...
	/*
	* Test 3
	* Update correction with old data. This should pause AMC correction.
	*/
	...
	/*
	* Test 4
	* Update correction with new GNSS data immediately. This should not 
	* resume correction as the correction must be paused for at least 3 
	* seconds before it can be resumed (see CORRECTION_PAUSE_MIN_TIME)
	*/
...
{
	/* 
	* Test that AMC correction fence distance input (mean distance and 
	* distance change) pause/resume AMC correction based on thresholds.
	*/
...
void test_correction_gnss_timeout(void)
{
	...
	*/
	...
	/*
	* Test
	* Test that correction is stopped when AMC receive a GNSS data event 
	* with the timed_out flag set to true.
	*/
	k_sem_reset(&warning_stop_sem);
