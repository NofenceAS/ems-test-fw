 ...
 * received from server is pushed to messaging module! */
{
    ...
    zassert_equal(err, 0, "Expected cellular_ack event was not "
                          "published ");
...
void test_ack_from_messaging_module_missed(void) /* happy scenario - msg
 * received from server is pushed to messaging module! */
{
    struct messaging_proto_out_event *test_msgIn =
            new_messaging_proto_out_event();
    ...
    int8_t err = k_sem_take(&cellular_proto_in, K_SECONDS(30));
    zassert_equal(err, 0, "Expected cellular_proto_in event was not "
                          "published ");
    ...
    zassert_not_equal(err, 0, "Unexpected cellular_proto_in event was "
                          "published ");
    err = k_sem_take(&cellular_proto_in, K_SECONDS(30));
...
{
//    int8_t test_msg_len = 123;
//    char test_pMsgIn;
//    struct cellular_proto_in_event *test_msgIn =
//            new_cellular_proto_in_event();
//    test_msgIn->buf = &test_pMsgIn;
//    test_msgIn->len = test_msg_len;
//    EVENT_SUBMIT(test_msgIn);
}
...
void test_socket_connect_fails(void)
{
    ...
    zassert_equal(err, 0, "Expected cellular_error event was not"
...
void test_socket_rcv_fails(void)
{
    ...
    int8_t err = k_sem_take(&cellular_ack, K_SECONDS(1));
    zassert_not_equal(err, 0, "Unexpected cellular_ack event was "
                          "published ");
    err = k_sem_take(&cellular_error, K_SECONDS(1));
    zassert_equal(err, 0, "Expected cellular_error event was not"
                              " published on receive error!");
...
void test_socket_connection_fails(void)
{
    test_init();
    received = -1;
    struct messaging_proto_out_event *test_msgIn =
...
void test_main(void)
{
	ztest_test_suite(
	        cellular_controller_tests, ztest_unit_test(test_init),
                  ...
                  ztest_unit_test(test_socket_connection_fails)
                  );
