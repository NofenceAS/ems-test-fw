 ...
 */
static int log_movement_out_event(const struct event_header *eh, char *buf, size_t buf_len)
{
...
EVENT_TYPE_DEFINE(movement_out_event, IS_ENABLED(CONFIG_LOG_MOVEMENT_EVENT), log_movement_out_event,
		  NULL);

static int log_movement_set_mode_event(const struct event_header *eh, char *buf, size_t buf_len)
{
	struct movement_set_mode_event *event = cast_movement_set_mode_event(eh);

	return snprintf(buf, buf_len, "Request to set movement mode %i", event->acc_mode);
}

EVENT_TYPE_DEFINE(movement_set_mode_event, IS_ENABLED(CONFIG_LOG_MOVEMENT_EVENT),
		  log_movement_set_mode_event, NULL);

EVENT_TYPE_DEFINE(activity_level, false, NULL, NULL);

EVENT_TYPE_DEFINE(step_counter_event, false, NULL, NULL);

static int log_movement_timeout_event(const struct event_header *eh, char *buf, size_t buf_len)
{
...
EVENT_TYPE_DEFINE(movement_timeout_event, IS_ENABLED(CONFIG_LOG_MOVEMENT_EVENT),
