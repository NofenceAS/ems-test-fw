 ...
 */
static int log_stg_ack_read_memrec_event(const struct event_header *eh,
					 char *buf, size_t buf_len)
{
...
EVENT_TYPE_DEFINE(stg_ack_read_memrec_event,
		  IS_ENABLED(CONFIG_STORAGE_EVENTS_LOG),
		  log_stg_ack_read_memrec_event, NULL);
 ...
 * @return return the number of characters read to the buffer
 */
static int log_stg_data_consumed_event(const struct event_header *eh, char *buf,
				       size_t buf_len)
{
	struct stg_data_consumed_event *event =
		cast_stg_data_consumed_event(eh);
	return snprintf(buf, buf_len, "Rotate fcb event: %p", event);
}

EVENT_TYPE_DEFINE(stg_data_consumed_event,
		  IS_ENABLED(CONFIG_STORAGE_EVENTS_LOG),
		  log_stg_data_consumed_event, NULL);
