...
#define BT_LE_ADV_NCONN_NAME_SLOW                                              \
	BT_LE_ADV_PARAM(/*BT_LE_ADV_OPT_CONNECTABLE | */                       \
			BT_LE_ADV_OPT_USE_NAME, BT_GAP_ADV_SLOW_INT_MIN,       \
...
static bool data_cb(struct bt_data *data, void *user_data)
{
	...
	if (data->type == BT_DATA_MANUFACTURER_DATA) {
		...
		if (data_len == BEACON_DATA_LEN) {
			...
			printk("Nofence beacon Major: %u Minor: %u RSSI: %u MANUF_ID: %u Beacon type: %u\n",
			       adv_data->major, adv_data->minor, adv_data->rssi,
			       adv_data->manuf_id, adv_data->beacon_dev_type);
		} else {
			memset(adv_data, 0, sizeof(*adv_data));
		}
		return false;
		// } else if (data->type == BT_DATA_SVC_DATA32) {
		// 	printk("Found service data\n");
		// 	return false;
	} else {
		return true;
	}
...
void nf_ble_init()
{
	...
	/* Start advertising */
	err = bt_le_adv_start(BT_LE_ADV_NCONN_NAME_SLOW, ad, ARRAY_SIZE(ad),
			      NULL, 0);
	if (err) {
