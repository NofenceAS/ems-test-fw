...
static int send_tcp_data(struct data *data)
{
    ...
    ret =  sendall(data->tcp.sock, data, data->tcp.expecting);
...
static int compare_tcp_data(struct data *data, const char *buf, uint32_t received)
{
    if (data->tcp.received + received > data->tcp.expecting) {
        LOG_ERR("Too much data received: TCP %s, expected only %d", data->proto, data->tcp.expecting);
//        return -EIO;
    }

    if (memcmp(buf, data->tcp.received, received) != 0) {
        LOG_ERR("Invalid data received:TCP, %s,  %d",buf, data->tcp.received);
//        return -EIO;
    }
...
static int process_tcp_proto(struct data *data)
{
    ...
    do {
        ...
        if(total_sends > 0)
        {
            ...
            ret = sendall(data->tcp.sock, buf, received);
        }
        else
        {
